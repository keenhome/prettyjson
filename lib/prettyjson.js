...
var addColorToData = function(input, options, isSchedule) {
  if (isSchedule) {
    const coloredSchedule = input.split(' ').map((scheduleBlock) => {
      if (scheduleBlock === 'occupied') {
        return colors.cyan(scheduleBlock)
      } else if (scheduleBlock === 'unoccupied') {
        return colors.yellow(scheduleBlock)
      } else {
        return scheduleBlock
      }
    })

    return coloredSchedule.join(' ')
  }
...
var renderToArray = function(data, options, indentation) {
  ...
  if (typeof data === 'string') {
    const isSchedule = includes(data, '\u001b[90m│\u001b[39m') && includes(data, 'Monday')

    return [
      Utils.indent(indentation),
      indentLines(addColorToData(data, options, isSchedule), indentation + options.defaultIndentation),
      Utils.indent(indentation)
    ];
  }
